...
# 화면 설정
SCREEN_WIDHT = SCREEN_HEIGHT = 750
...
def main():
    ...
    def redraw_window():
        ...
        WIN.blit(level_label, (SCREEN_WIDHT-level_label.get_width()-10, 10))
        ...
        if lost:
            ...
            WIN.blit(lost_label,
                     (SCREEN_WIDHT / 2 - lost_label.get_width() / 2, 350))
    ...
    while run:
        ...
        if keys[pygame.K_s] and player.y <SCREEN_HEIGHT-player.get_height():
            player.y += player_vel
        if keys[pygame.K_d] and player.x < SCREEN_WIDHT-player.get_width():
            player.x += player_vel


        #적군 비행서 초기화(비행선 목록이 빈 경우만 실행)
        if len(enemies) == 0:
            level += 1
            wave_length += 5
            for x in range(wave_length):
                enemy =Enemy(random.randint(50, SCREEN_WIDHT-100),
